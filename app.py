...
def registrar():
    if request.method == "POST":
        ...
        exito = f_agregar_cliente(nombre, ap_paterno, ap_materno, correo, telefono)
        
        if exito:
            return redirect(url_for("mostrar_clientes"))
        else:
            return "<h3>Error: No se pudo registrar el cliente. Comprueba que el correo no esté duplicado.</h3><br><a href='/registro'>Volver a intentar</a>", 400
